<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="frm_IDParse.aspx.cs" Inherits="CSharpGoWeb.frm_IDParse" %>
...
<STYLE>
div#num2AreaList{display:none}
</STYLE>
<script type="text/javascript">
$( document ).ready(function() {
 
    $( "#btnParse" ).click(function( event ) {
        var num2AreaList =  $( "#num2AreaList" ).html().trim();
        var arrayArea = num2AreaList.split("\n"); 
        var num2AreaListDic = new Array();
        for(var i = 0; i < arrayArea.length;i++)
        {
            var strTemp = arrayArea[i];
            var key2Value = strTemp.split(" ");
            num2AreaListDic[key2Value[0]] = key2Value[1];
        }
        var id =  $( "#inputID" ).val();
        var areaCode = id.substr(0,6);
        $( "#inputArea" ).val(num2AreaListDic[areaCode]);
        var strBirthday = id.substr(6,4);
        var time = new Date();
        var date =  time.getFullYear();
        var age = date - strBirthday;
        $( "#inputBirth" ).val(age);
        var nSex = id.substr(16,1);
        var strSex = "男";
        if(nSex % 2 == 0)
            strSex = "女";
        $( "#Sex" ).val(strSex);
            ...
            <fieldset>
                ...
                <div class="control-group">
                    <label class="control-label" for="input01">
                        请输入身份证号</label>
                    <div class="controls">
                        <input type="text" class="input-xlarge" id="inputID">
                        <p class="help-block">
                            身份证号</p>
                    </div>
                </div>
                <div class="control-group">
                    <label class="control-label" for="input01">
                        地域</label>
                    <div class="controls">
                        <input type="text" class="input-xlarge" id="inputArea">
                        <p class="help-block">
                           所在地</p>
                    </div>
                </div>
                <div class="control-group">
                    <label class="control-label" for="input01">
                        年龄</label>
                    <div class="controls">
                        <input type="text" class="input-xlarge" id="inputBirth">
                        <p class="help-block">
                            年龄</p>
                    </div>
                </div>
                  <div class="control-group">
                    <label class="control-label" for="input01">
                        性别</label>
                    <div class="controls">
                        <input type="text" class="input-xlarge" id="Sex">
                        <p class="help-block">
                            性别</p>
                    </div>
                </div>
                <div class="form-actions">
                    <button class="btn btn-primary" type="button" id="btnParse">
                        解析</button>
                </div>
...
659001 新疆维吾尔族自治区石河子市  
        ...
        <div class="span4">
            ...
            <ul>
                <li>JQuery ID选择器</li>
                <li>javascript 数组，字典，字符串分割函数，字符串取字串函数应用</li>
                <li>BootStrap栅格布局，表单控件</li>
            </ul>
            <hr>
            <h3>
                身份证号码解析规则</h3>
            <p>
                <span  style="color:Blue">地址码</span>（身份证前六位）表示编码对象常住户口所在县(市、旗、区)的行政区划代码。
            </p>
            <p>
                <span  style="color:Blue">生日期码</span> （身份证第七位到第十四位）表示编码对象出生的年、月、日，其中年份用四位数字表示，年、月、日之间不用分隔符。例如：1981年05月11日就用19810511表示。
            </p>
            <p>
                <span  style="color:Blue">顺序码</span> （身份证第十五位到十七位）为同一地址码所标识的区域范围内，对同年、月、日出生的人员编定的顺序号。其中第十七位奇数分给男性，偶数分给女性。
            </p>
            <p>
                <span  style="color:Blue">校验码</span> （身份证最后一位）是根据前面十七位数字码，按照ISO 7064:1983.MOD 11-2校验码计算出来的检验码。
            </p>
        </div>
    </div>
</body>
